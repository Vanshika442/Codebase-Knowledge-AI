...
st.markdown(
    ...
    <style>
      ...
      .app-sub {color: #9CA3AF; margin-bottom: 1rem;}
      .kpi {
      ...
      .kpi-value {font-size: 1.25rem; font-weight: 600; margin-top: 6px;}
      .section-title {font-size: 1.1rem; font-weight: 600; margin: .4rem 0 .7rem;}
...
<style>
  ...
  .app-title {
    font-size: 2.1rem;
    font-weight: 800;
    line-height: 1.25;   /* prevents clipping */
  ...
  .hint {color:#A78BFA; font-size: .88rem;}
...
with st.sidebar:
    ...
    show_ast = st.toggle("Show AST hints", value=True)
    st.markdown("---")
...
with col1:
    ...
    st.markdown(f'<div class="kpi"><div class="kpi-label">Active Repo ID</div><div class="kpi-value">{value}</div></div>', unsafe_allow_html=True)
with col2:
    files_loaded = "-"
    if st.session_state.index_summary:
        files_loaded = st.session_state.index_summary.get("files_loaded", "-")
    st.markdown(f'<div class="kpi"><div class="kpi-label">Indexed Files</div><div class="kpi-value">{files_loaded}</div></div>', unsafe_allow_html=True)
with col3:
    chunks = "-"
    if st.session_state.index_summary:
        chunks = st.session_state.index_summary.get("chunks_indexed", "-")
    st.markdown(f'<div class="kpi"><div class="kpi-label">Indexed Chunks</div><div class="kpi-value">{chunks}</div></div>', unsafe_allow_html=True)
with col4:
    latency = st.session_state.query_latency
    latency_text = f"{latency:.2f}s" if isinstance(latency, (int, float)) else "-"
    st.markdown(f'<div class="kpi"><div class="kpi-label">Last Query Latency</div><div class="kpi-value">{latency_text}</div></div>', unsafe_allow_html=True)

st.markdown("")

# ---------- Main Tabs ----------
tab_index, tab_query, tab_sources = st.tabs(["Index Repository", "Ask Questions", "Index Details"])

with tab_index:
    st.markdown('<div class="section-title">Repository Indexing</div>', unsafe_allow_html=True)
    st.markdown('<div class="hint">Enter a local path or GitHub URL. Existing repo is pulled if already cloned.</div>', unsafe_allow_html=True)
        ...
        if not repo_input.strip():
            ...
            with st.spinner("Indexing repository. This may take a while on CPU..."):
                try:
                    ...
                    st.success("Index completed successfully.")
                except Exception as e:
                    st.exception(e)

with tab_query:
    st.markdown('<div class="section-title">Codebase Q&A</div>', unsafe_allow_html=True)
    st.markdown('<div class="hint">Ask architecture, flow, function, and module-level questions.</div>', unsafe_allow_html=True)
    ...
    question = st.text_area(
        "Question",
        height=130,
        placeholder="How does authentication flow work across files?",
    )
    ...
    if ask_btn:
        if not repo_id.strip():
            ...
            with st.spinner("Retrieving context and generating answer..."):
                try:
                    result = answer_question(repo_id=repo_id.strip(), question=question.strip(), top_k=top_k)
    ...
    if st.session_state.last_answer:
        ...
        if show_ast and result.get("ast_hints"):
            st.markdown("#### AST Hints")
            for h in result.get("ast_hints", []):
                ...
                el = h.get("end_line")
                if sl and el:
                    st.write(f"- {typ}: {name} ({file_}:{sl}-{el})")
                else:
                    st.write(f"- {typ}: {name} ({file_})")
...
with tab_sources:
    ...
    if st.session_state.index_summary:
        ...
        if indexed_list:
            ...
            st.dataframe({"file": indexed_list}, use_container_width=True, hide_index=True)
    else:
        st.info("No index summary yet. Build index first.")
